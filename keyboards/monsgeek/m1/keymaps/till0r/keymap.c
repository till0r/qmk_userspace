...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_B] = LAYOUT_all( /* Base Layer */
        ...
        KC_LCTL, KC_LGUI, KC_LALT,                   KC_SPC,                             KC_RALT, MO(WIN_FN),KC_LCTL,        KC_LEFT, KC_DOWN, KC_RGHT),

    [WIN_FN] = LAYOUT_all( /* Function Layer */
        _______, _______, _______, KC_WSCH, KC_WHOM, KC_MSEL, KC_MPLY, KC_MPRV, KC_MNXT, _______, _______, _______, _______, KC_DEL,            RM_NEXT,
    ...
    switch (keycode) {
        case KC_F1:
            if (record->event.pressed && (mods & MOD_MASK_SHIFT)) {
                // Temporarily clear Shift so KC_1 is not sent as !
        ...
        case KC_F2:
            if (record->event.pressed && (mods & MOD_MASK_SHIFT)) {
                // Temporarily clear Shift so KC_2 is not sent as @
                del_mods(MOD_MASK_SHIFT);
                send_keyboard_report();
